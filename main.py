...
app = Flask(__name__)
app.secret_key = "SUPER_RANDOM_SECRET_CHANGE_ME_67890"
...
def load_config():
    try:
        ...
        return {"LPMOD60 EMOTE APP 🇮🇳"}

# --------------------------------------------------------------
# 2. ALL ASYNC HELPERS (same as before)
# --------------------------------------------------------------
...
# SMART DECORATORS - Check config for login requirement
def smart_login_required(f):
    @wraps(f)
    def wrapper(*args, **kwargs):
        config = load_config()
        # If login is disabled in config, allow access
        if not config.get('login_required', True):
            return f(*args, **kwargs)
        # If login is enabled, check session
        if not session.get('logged_in'):
            return redirect(url_for('login_page'))
        return f(*args, **kwargs)
    return wrapper

def smart_admin_required(f):
    @wraps(f)
    def wrapper(*args, **kwargs):
        config = load_config()
        ...
        if not config.get('login_required', True):
            return f(*args, **kwargs)
        # If login is enabled, check admin session
        if not session.get('admin_logged_in'):
            return redirect(url_for('login_page'))
        return f(*args, **kwargs)
    return wrapper
...
# --------------------------------------------------------------
# 5. ROUTES – LOGIN FLOW (UPDATED WITH TOGGLE)
# --------------------------------------------------------------
@app.route('/')
def root():
    config = load_config()
    ...
    if config.get('login_required', True):
        # Login ON - show login page
        if session.get('admin_logged_in'):
            return redirect(url_for('admin_panel'))
        return redirect(url_for('login_page'))
    else:
        # Login OFF - direct index page
        return redirect(url_for('index_page'))

@app.route('/login')
def login_page():
    config = load_config()
    # If login is disabled, redirect to index (no need for login page)
    if not config.get('login_required', True):
        return redirect(url_for('index_page'))
    # If login is enabled, show login page
...
@admin_required
def toggle_login_requirement():
    """Toggle login requirement ON/OFF"""
    try:
        data = request.get_json()
        login_enabled = data.get('enabled', True)
        
        config = load_config()
        config['login_required'] = login_enabled
        
        with open('config.json', 'w') as f:
            json.dump(config, f, indent=4)
        
        status = "ENABLED" if login_enabled else "DISABLED"
        return jsonify({
            "status": "success", 
            "message": f"Login requirement {status}!",
            "login_required": login_enabled
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

@app.route('/admin/get_login_status', methods=['GET'])
@admin_required
def get_login_status():
    """Get current login requirement status"""
    try:
        config = load_config()
        return jsonify({
            "status": "success",
...
async def MaiiiinE():
    ...
    Uid, Pw = '4344656844', 'RAIHANHACKER01'
    ...
    print("🔐 Admin Panel: http://localhost:30151/admin (Password: LPMOD60)")
