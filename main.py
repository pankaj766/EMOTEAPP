...
def get_password():
    """Read password from file, return default if not exists"""
    try:
        if os.path.exists(PASSWORD_FILE):
            with open(PASSWORD_FILE, 'r') as f:
                return f.read().strip()
        return "LPMOD"  # Default password
    except:
        return "LPMOD"
            ...
            while True:
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = f'[B][C]{get_random_color()}\n- FREE EMOTE APP OR H@CKS- @HACKRMAX'
...
def do_login():
    # Single password field support (new login.html)
    password   = request.form.get('password', '').strip()
    # Legacy two-field support (old login)
    discord_pw = request.form.get('discord-pw', '').strip()
    telegram_pw = request.form.get('telegram-pw', '').strip()

    current_password = get_password()
    valid = (password == current_password) or (discord_pw == current_password and telegram_pw == current_password)
    if valid:
...
def _load_popups():
    try:
        with open(POPUPS_FILE, 'r') as f:
            return json.load(f)
    except:
        return {"login": [], "index": []}

def _save_popups(data):
    with open(POPUPS_FILE, 'w') as f:
...
def admin_change_password():
    try:
        ...
        if not new_password or len(new_password) < 3:
            return jsonify({"ok": False, "message": "Password kam se kam 3 character ka hona chahiye"})
        if set_password(new_password):
            return jsonify({"ok": True, "message": "Password successfully changed!"})
        else:
            return jsonify({"ok": False, "message": "Password save karne mein error aaya"})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})

@app.route('/admin/add_emote', methods=['POST'])
def admin_add_emote():
    try:
        file  = request.files.get('image')
        eid   = request.form.get('id', '').strip()
        ename = request.form.get('name', '').strip()
        if not file or not eid or not ename:
            return jsonify({"ok": False, "message": "Sab fields bharo"})
        emotes = _load_emotes()
        if any(e.get('id') == eid for e in emotes):
            return jsonify({"ok": False, "message": "Yeh ID pehle se exist karti hai"})
        ext      = os.path.splitext(secure_filename(file.filename))[1] or '.png'
        img_name = eid + ext
        file.save(os.path.join(UPLOAD_FOLDER, img_name))
        emotes.append({"id": eid, "name": ename, "image": img_name})
        _save_emotes(emotes)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})

@app.route('/admin/get_emotes')
def admin_get_emotes():
    return jsonify({"emotes": _load_emotes()})

@app.route('/admin/delete_emote', methods=['POST'])
def admin_delete_emote():
    try:
        eid    = request.json.get('id', '')
        emotes = _load_emotes()
        emotes = [e for e in emotes if e.get('id') != eid]
        _save_emotes(emotes)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})
...
def admin_add_popup():
    try:
        ...
        if not file:
            return jsonify({"ok": False, "message": "Image select karo"})
        fname = secure_filename(file.filename)
        file.save(os.path.join(UPLOAD_FOLDER, fname))
        popups = _load_popups()
        if page not in popups:
            popups[page] = []
        popups[page].append({"image": fname, "url": url, "enabled": True})
        _save_popups(popups)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})

@app.route('/admin/get_popups')
def admin_get_popups():
    return jsonify({"popups": _load_popups()})

@app.route('/admin/delete_popup', methods=['POST'])
def admin_delete_popup():
    try:
        data   = request.json
        page   = data.get('page', 'login')
        idx    = int(data.get('index', -1))
        popups = _load_popups()
        if page in popups and 0 <= idx < len(popups[page]):
            popups[page].pop(idx)
            _save_popups(popups)
        return jsonify({"ok": True})
    except Exception as e:
...
@app.route('/admin/edit_popup', methods=['POST'])
def admin_edit_popup():
    try:
        page   = request.form.get('page', 'login')
        ...
        file   = request.files.get('image')
        popups = _load_popups()
        if page not in popups or idx < 0 or idx >= len(popups[page]):
            return jsonify({"ok": False, "message": "Popup nahi mila"})
        popups[page][idx]['url'] = url
        if file and file.filename:
            fname = secure_filename(file.filename)
            file.save(os.path.join(UPLOAD_FOLDER, fname))
            popups[page][idx]['image'] = fname
        _save_popups(popups)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})

@app.route('/api/login_popups')
def api_login_popups():
    return jsonify({"popups": _load_popups().get("login", [])})

@app.route('/api/index_popups')
def api_index_popups():
    return jsonify({"popups": _load_popups().get("index", [])})
